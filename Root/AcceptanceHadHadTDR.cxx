...
EL::StatusCode AcceptanceHadHadTDR :: histInitialize ()
{
  ...
  hists["l1_symmetric"] = new TH1F("l1_symmetric", "l1_symmetric", nsteps, l1_min, l1_min + step * nsteps);
  map_l1 =  new TH2F("l1_asymmetric", "l1_asymmetric", 
		     nsteps, l1_min, l1_min + step * nsteps,
		     nsteps, l1_min, l1_min + step * nsteps);

  for (int i = 0; i < nsteps; i++) {
    int thresh = (int) (l1_min / 1000 + step * i / 1000);
    std::cout << i << ": " << thresh << std::endl;
  ...
  hists["jet1_eta"] = new TH1F("h_jet1_eta", "jet1_eta", 10, -2.5, 2.5);
  hists["jet2_eta"] = new TH1F("h_jet2_eta", "jet2_eta", 10, -2.5, 1.5);
...
EL::StatusCode AcceptanceHadHadTDR :: execute ()
{
  ...
  for (int i = 0; i < nsteps; i++) {
    ...
    for (int j = i; j < nsteps; j++){
      int sub_thresh = (int) (l1_min / 1000 + step * j / 1000);
  ...
  ATH_MSG_INFO("Fill kinematics histograms:");
...

{
  ...
  for (const auto tau: *taus) {
    ...
    // pt cut
    if (tau->pt() < 20000.) {
      ATH_MSG_VERBOSE("Reject tau " << tau->index() << " with pt = " << tau->pt());
      continue;
    }

    // eta cut
    if (fabs(tau->eta()) > 2.5) {
      ATH_MSG_VERBOSE("Reject tau " << tau->index() << " with eta = " << tau->eta());
      continue;
    }
    ...
    if (tau->nTracks() != 1 and tau->nTracks() != 3) {
      ATH_MSG_VERBOSE("Reject tau " << tau->index() << " with nTracks = " << tau->nTracks());
      continue;
    }
    // ID cut
    if (not tau->isTau(xAOD::TauJetParameters::JetBDTSigMedium)) {
      ATH_MSG_VERBOSE("Reject tau " << tau->index() << " with medium ID = " << tau->isTau(xAOD::TauJetParameters::JetBDTSigMedium));
      continue;
    }      
      // selectDec(*tau) = true;
